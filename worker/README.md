...
## 4б. Эндпоинт `/fetch-link` (проверка сдач-ссылок)

Если студент сдал не файл, а ссылку (Google Диск, Яндекс Диск, Dropbox,
прямая ссылка на PDF), кнопка «✏️ Проверить» в админке просит воркер
скачать PDF по этой ссылке (`POST /fetch-link { token, cid, url }`) и
открывает его в проверялке. Дополнительных переменных не нужно, только
перевыкатить `worker.js` (п. 2). Пока воркер не обновлён, кнопка
предложит скачать файл вручную и выбрать его с диска.

Файл на Google Диске должен быть открыт «всем, у кого есть ссылка»;
Google Docs / Slides / Sheets экспортируются в PDF автоматически.

## 4в. Эндпоинт `/lab-progress` (резервная копия закрытых лаб в git)

Прогресс закрытых лабораторных (отметки автопроверки и код студента из
всех редакторов) живёт в Firestore `lab_progress/{uid}_{cid}_{lab}`.
Чтобы он не зависел от одной базы, воркер дублирует его в приватный репо
сдач `GITHUB_REPO` в папку `labs/{cid}/{lab}/{uid}/`:
`{editorId}.cpp` — код по задачам, `progress.json` — отметки, зачёт,
email/ФИО. Все файлы одного запроса — один коммит; если ничего не
изменилось, коммит не создаётся. История правок кода студента — в git log.

Кто и когда шлёт:
- страница лабы (`js/labs.js`, `CFDLabs.saveProgress`) — при каждом
  сохранении прогресса полный снимок (весь код + все отметки), но не чаще
  раза в минуту на студента; отложенный снимок досылается по таймеру и
  сразу при уходе со вкладки; ошибки глушатся, на студента не влияют;
- админка, вкладка «Лабы», кнопка «⇩ выгрузить в git» — все студенты
  выбранной лабы разом (нужен admin курса: `SUPERADMINS` или
  `COURSE_ADMINS_JSON` с этим курсом, например `"nm"`).

Дополнительные переменные не нужны; опционально `GITHUB_BRANCH` — ветка
приватного репо (по умолчанию берётся default branch репозитория).
Нужно перевыкатить `worker.js` (п. 2); пока воркер не обновлён, копии
просто не создаются, всё остальное работает как раньше.
