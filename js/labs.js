  ...
  const API = {
    ...
    mountLab: async function (opts) {
      ...
      const tryOpen = async () => {
        ...
        let keyB64 = null, keyErr = null;
        try { keyB64 = await API.getKey(opts.cid, opts.lab); }
        catch (e) { keyB64 = null; keyErr = e; }
        if (!keyB64) {
          if (isCourseAdmin && !keyErr) {
            // Админ курса ключ читать вправе — значит записи просто нет.
            showMsg("⚙ Ключ лабы не внесён", "Вы админ курса, но в Firestore нет ключа <code>lab_keys/" + opts.cid + "_" + opts.lab + "</code>. Откройте админку → вкладка «Лабы» и вставьте ключ этой лабораторной (он печатается скриптом tools/lab_encrypt.py).");
          } else if (keyErr) {
            showMsg("⏳ Лаба закрыта", "Ключ заданий недоступен. Если вы преподаватель — проверьте, что у вашего профиля стоит isAdmin и курс «" + opts.cid + "» есть в managedCourses. Если вы студент — лабораторную открывает преподаватель на занятии для отмеченных присутствующими.");
          } else {
